...
class EWeLinkTrawler extends SiteTrawler {
   ...
   * @param {boolean}  params.alwaysReportIfOff - Optional  arg. Default False. If set, it will report when the device is off.
  ...
  constructor(params) {
    ...
    this.alwaysReportIfOff = params.alwaysReportIfOff | false
  }
...
method.getResultsString = function () {
  ...
  const ret = this.results.reduce( (accumulator, r) => {
    return `${accumulator}<br>${r.name}: Battery ${r.battery}, Status ${r.status}`
  }, "")
...
method.resultPassesCustomFilters = function (params) {
  ...
  if (this.alwaysReportIfOff && r.status == "off") return true;
