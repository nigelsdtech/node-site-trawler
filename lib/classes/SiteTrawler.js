...
method.getResults = async function (params) {
  ...
  const oldHighestSeenId = (() => {
    if (!rhsi) return null;
    if (!self.savedData.highestSeenId) return -1
  ...
  const parseResults = async (accumulator) => {
    ...
    const test2 = await self.resultPassesCustomFilters({result: result})
      .catch((err) => {
    ...
    return parseResults ({
      ...
      seenIds: newSeenIds,
      filteredResults: (isQualified)? filteredResults.concat(result) : filteredResults
    })
