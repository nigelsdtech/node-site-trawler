...
method.getResults = function (params,cb) {
  ...
  self.loadResults(null,function (err) {
    ...
    var highest
    var rhsi = self.recordHighestSeenId

    if (rhsi) {
      highest = (self.savedData.highestSeenId)? self.savedData.highestSeenId : -1;
    }
    var oldHsi = highest


    // Tracking all seen Ids
    var rasi = self.recordAllSeenIds
    var oldSeenIds = (self.savedData.seenIds)? self.savedData.seenIds : [];
    var newSeenIds =      []
    var filteredResults = []
    ...
    for (var i = 0; i < self.results.length; i++) {
      ...
      if (rhsi && result.id > highest) { highest = result.id }

      // Apply various filters to the returned results
      if (
        self.resultPassesCommonFilters({result: result}) &&
        self.resultPassesCustomFilters({result: result})
      ) {
        filteredResults.push(result)
        if (rasi) newSeenIds.push(result.id) ;
      }

      // Quit if we've hit the upper limit
      if (self.maxResults && filteredResults.length == self.maxResults) { break }

    }

    if (rhsi && oldHsi != highest) {
      self.dataToSave.highestSeenId = highest
    }
