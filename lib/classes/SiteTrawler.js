...
method.getResults = function (params,cb) {
  ...
  self.loadResults(null,function (err) {
    ...
    if (self.recordHighestSeenId || self.recordAllSeenIds) {
      ...
      var seen    = (self.savedData.seenIds)?       self.savedData.seenIds       : [];
      ...
      if (self.recordAllSeenIds && seen.length > 0) {
        self.dataToSave.seenIds = seen
      }
    }


    self.log('debug', 'Results in order - ' + JSON.stringify(self.results))
