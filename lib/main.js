var cfg                = require('config'),
    ...
    TwitterTrawler     = require('./classes/TwitterTrawler.js');
...
module.exports = function (params, programComplete) {
  ...
  for (var i = 0; i < cfg.trawlers.length; i++) {
    ...
    if (trawler.trawlModel == "twitter") {
      var t = cfg.trawlers[i]
      var tt = new TwitterTrawler(t.setup)
      if (savedData[t.setup.id]) { tt.setSavedData({savedData: savedData[t.setup.id]}) }
      trawlers.push(tt)
    }
