/**
 * This object represents a CentralTickets searcher. It gets the tweet timeline for a particular user and searches for content I'm interested in.
 */
...
import { websiteScraperConstructorArgs } from '../interfaces/iGeneral';
import {aSiteTrawler} from './aSiteTrawler';



/**
 * Basic request params for all calls to CentralTickets
 */
const siteRequest = request.defaults({
  baseUrl: 'https://www.centralTickets.com',
  timeout: cfg.connectionTimeout || null
})

type centralTicketsTrawlerResult = {
  id: number,
  title: string,
  url: URL,
  price: number
...
class CentralTicketsTrawler extends aSiteTrawler {

  private siteQuery: string;

  /**
   * CentralTicketsTrawler model constructor.
   ...
   * @param {object}  params.siteQueryUrl     - Search query filters to be sent to centralTickets.
   ...
   * @param {string}  params.regexMatchField - The field against which to run the regex
   * @constructor
   */
  constructor(params: websiteScraperConstructorArgs) {

    const superArgs = Object.assign ({}, params, {
      regexMatchField : "title",
      recordAllSeenIds : true
    })

    super(superArgs)

    this.siteQuery = params.siteQuery
  }

  /**
   * CentralTicketsTrawler.getResultsString
   *
   ...
   *
   * @alias CentralTicketsTrawler.getResultString
   *
   */
  getResultsString = function () {

    const ret = this.results.reduce((accumulator, result: centralTicketsTrawlerResult) => {
      return accumulator
      + "<p>"
      + "<br>" + result.title
      + "<br>" + result.url.href
...
var method = CentralTicketsTrawler.prototype




/**
 * CentralTicketsTrawler.loadResults
 *
 * @desc Get a set of centralTickets results
 *
 * @alias CentralTicketsTrawler.loadResults
 *
...
method.loadResults = function (params,cb) {
  ...
  siteRequest.get({
    ...
    for (var i = (listings.length - 1); i >= 0; i--) {
      ...
      result.url = "https://www.centralTickets.com" + $(listing).attr("href").trim()
...
module.exports = CentralTicketsTrawler
